...
def validate_usability(contract,output,errors,warnings):
 ...
 declared=set(arts)|{str(x.get('assetId')) for x in contract.get('locks',{}).get('characters',[]) if isinstance(x,dict) and x.get('assetId')}
 declared_occ=[]; refs=[]
 for spec in contract.get('artifactQueue',[]):
  ...
  if spec.get('required',True) and not str(spec.get('repairAction') or spec.get('repair','')).strip(): issue(errors,'ARTIFACT_REPAIR_MISSING',aid,artifactId=aid,gate=_gate_for(contract,aid))
